...
let suite = 
  describe "Bmap" (fun () -> [
    test "size empty" (fun () -> 
      empty |> size |> Expect.toEqual 0);
    test "size map0" (fun () -> 
      map0 |> size |> Expect.toEqual 1);

    test "get - success" (fun () -> 
      map0 |> get key1 |> Expect.toEqual (Some "one"));
    test "get - fail" (fun () -> 
      map0 |> get key2 |> Expect.toEqual None);

    test "getOr - success" (fun () -> 
      map0 |> getOr key1 ~default:"default" |> Expect.toEqual "one");
    test "getOr - fail" (fun () -> 
      map0 |> getOr key2 ~default:"default" |> Expect.toEqual "default");

    test "update - does not touch other cells" (fun () -> 
      let map1 = map0 |> update key2 (function _ -> None) in
      map1 |> get key1 |> Expect.toEqual (Some "one"));
    test "update - Some -> Some" (fun () -> 
      let map1 = map0 |> update key1 (function | Some _ -> Some "two" | v -> v) in
      map1 |> get key1 |> Expect.toEqual (Some "two"));
    test "update - None -> Some" (fun () -> 
      let map1 = map0 |> update key2 (function | None -> Some "two" | v -> v) in
      map1 |> get key2 |> Expect.toEqual (Some "two"));
    test "update - Some -> None" (fun () -> 
      let map1 = map0 |> update key1 (function _ -> None) in
      map1 |> get key1 |> Expect.toEqual None);
    ...
    test "fromList" (fun () ->
      ...
      map1 |> get key3 |> Expect.toEqual (Some "three")
    );

    test "toList" (fun () ->
      map0 |> toList |> Expect.toEqual [key1, "one"]);

    test "fromSeq" (fun () ->
      let map1 = fromSeq (Sequence.singleton (key1, "hello")) in
      map1 |> size |> Expect.toEqual 1);

    test "toSeq" (fun () ->
      map0 |> toSeq |> Sequence.to_list |> Expect.toEqual (map0 |> toList));

    test "keysList" (fun () ->
      map0 |> keysList |> Expect.toEqual [key1]);

    test "valuesList" (fun () ->
      map0 |> valuesList |> Expect.toEqual ["one"]);
  ])