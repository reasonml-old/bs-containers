...
let suite =
  describe "Barray" (fun () -> [
    ...
    test "Barray.reduceWhile" (fun () ->
      [|1; 2; 3|] |> Barray.reduceWhile (fun acc x -> if x > 4 then (acc, true) else (acc + x, false)) 0
                  |> Expect.toEqual 6
    );
    test "Barray.reduceWhile stop" (fun () ->
      [|1; 2; 3; 4|] |> Barray.reduceWhile (fun acc x -> if x >= 3 then (acc, true) else (acc + x, false)) 0
                     |> Expect.toEqual 3
    );
