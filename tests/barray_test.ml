...
let suite =
  describe "Barray" (fun () -> [
      ...
      test "Barray.slice 3" (fun () -> 
        [|1; 2; 3|] 
        |> Barray.slice ~start_:0 ~end_:100
        |> Expect.toEqual None
      );
      test "Barray.slice 4" (fun () ->
        [|1; 2; 3; 4|] 
        |> Barray.slice ~start_:1 ~end_:2 
        |> Expect.toEqual (Some [| 2 |])
      );
      test "Barray.slice 5" (fun () -> 
        [|1; 2; 3; 4|] 
        |> Barray.slice ~start_:1 ~end_:3
        |> Expect.toEqual (Some [| 2; 3 |])
      );

      test "Barray.slice 6" (fun () -> 
        [|1;2;3;4|] 
        |> Barray.slice ~start_:0 ~end_:4
      ...
      test "Barray.mapWithIndex" (fun () ->
        [|1; 2; 3|] 
        |> Barray.mapWithIndex (fun i x -> i + x)
      ...
      test "Barray.map2" (fun () -> 
        let c = Barray.map2 
          (fun x y -> Bchar.getDigitOrRaise x + y) [|'1'; '2'; '3'|] [|1; 2; 3|]
      ...
      test "Barray.reduceReversed reverse" (fun () -> 
        [|1; 2; 3|] 
        |> Barray.reduceReversed (fun x acc -> acc @ [x]) []
      ...
      test "Barray.reduceWithIndex" (fun () ->
        [|1; 2; 3|] 
        |> Barray.reduceWithIndex (fun acc index x -> acc + index + x) 0
        |> Expect.toEqual 9
      );
      ...
      test "Barray.find" (fun () ->
        [|1; 2; 3; 4|] 
        |> Barray.find (fun x -> if x == 2 then Some x else None) 
        |> Expect.toEqual (Some 2)
      );
      ...
      test "Barray.findWithIndex" (fun () ->
        [|4; 3; 2; 1; 5|] 
        |> Barray.findWithIndex (fun i x -> if i + x = 4 then Some(x, i) else None) 
      ...
      test "Barray.findIndex" (fun () ->
        [|5; 4; 3; 1; 2|] 
        |> Barray.findIndex(fun x -> if x = 4 then true else false) 
        |> Expect.toEqual @@ Some (1, 4)
      );
      test "Barray.findIndex - not found" (fun () -> 
        [|5; 4; 3; 1; 2|] 
        |> Barray.findIndex (fun x -> if x = 7 then true else false) 
        |> Expect.toEqual None
      );
      ...
      test "Barray.forAll" (fun () -> 
        [|1; 2; 3|] 
        |> Barray.forAll (fun x -> x < 4)
        |> Expect.toEqual true
      );
      test "Barray.forAll false" (fun () -> 
        [|1; 2; 3|] 
        |> Barray.forAll (fun x -> x < 2) 
        |> Expect.toEqual false
      );
      ...
      test "Barray.filterMap" (fun () -> 
        [|1; 2; 3; 4|]
        |> Barray.filterMap (fun x -> if x mod 2 = 0 then Some x else Some (x * 2))
