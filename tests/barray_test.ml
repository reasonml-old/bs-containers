...
let suite =
  describe "Barray" (fun () -> [
        ...
        test "Barray.blit empty empty" (fun () -> 
            let a = [||] in
            let b = [||] in
            let _ = Barray.blit a 0 b 0 0 in 
            Expect.toEqual b [||]
          );
        test "Barray.blit empty empty long" (fun () -> 
            let a = [||] in
            let b = [||] in
            Expect.toRaise (fun () -> Barray.blit a 0 b 0 100)
          );
        test "Barray.blit" (fun () -> 
            let a = [|1; 2; 3; 4|] in
            let b = [|5; 6; 7; 8|] in
            let _ = Barray.blit a 0 b 0 4 in
            Expect.toEqual b [|1; 2; 3; 4|]
          );
        test "Barray.blit self" (fun () ->
            let a = [|1; 2; 3; 4|] in
            let _ = Barray.blit a 0 a 2 2 in
