...
let filter ~f l =
  let rec direct i f l = match l with
    | [] -> []
    | _ when i=0 -> safe f l []
    | x::l' when not (f x) -> direct i f l'
    | x::l' -> x :: direct (i-1) f l'
...
let rec fold_while ~f ~init = function
  | [] -> init
  | e::l -> let acc, cont = f init e in
    match cont with
    | `Stop -> acc
    | `Continue -> fold_while ~f ~init l
...
let fold_map ~f ~init l =
  let rec aux f init map_acc l = match l with
    | [] -> init, List.rev map_acc
    | x :: l' ->
      let init, y = f init x in
      aux f init (y :: map_acc) l'
  in
  aux f init [] l
...
let remove ?(eq=(=)) ~key l =
  let rec remove' eq key acc l = match l with
    | [] -> List.rev acc
    | y :: tail when eq key y -> remove' eq key acc tail
    | y :: tail -> remove' eq key (y::acc) tail
  in
  remove' eq key [] l
...
let foldi ~f ~init l =
  let rec foldi f init i l = match l with
    | [] -> init
    | x::l' ->
      let acc = f init i x in
      foldi f acc (i+1) l'
  in
  foldi f init 0 l
